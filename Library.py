...
class LibraryApp:
    def __init__(self):
        self.system = self._load_or_create()
        self.start()
    ...
    def _load_or_create(self):
        try:
            with open(DATA_FILE, "rb") as f:
                ...
                if not isinstance(system, LibrarySystem):
                    raise TypeError("Неверный тип данных в файле")
                if system.data_version < DATA_VERSION:
                    print("Обнаружена устаревшая версия данных. Выполняется обновление.")
                return system
        except (FileNotFoundError, pickle.PickleError, EOFError, TypeError) as e:
            print(f"Не удалось загрузить сохранённые данные: {e}")
            print("Создаётся новая библиотечная система.")
